...
def emit_kbars():
    """抓 1 分鐘 K → resample 成日 K，只送最近 30 根"""
    ...
    df = pd.DataFrame({**kbars_min})
    df.ts = pd.to_datetime(df.ts)                    # 轉 Timestamp

    # 以每天 09:00 為開盤 resample 成日 K
    daily = (
        df.set_index("ts")
    ...
    daily = daily.drop_duplicates(subset="ts").sort_values("ts")  # ← 新增
    daily["ts"] = (daily["ts"].view("int64") // 1_000_000).astype(int)

    print("sample bar →", daily.iloc[0].to_dict())   # 應該是 13 位
...
# === 5. 市場推送狀態 =======================================
def emit_price(p: float):
    sio.emit("priceUpdate", {"ts": int(time.time()*1000), "price": p}, namespace="/")
...
def emit_mkt():
    ...
    # TSE 僅保留 last
    if "TSE" in data:
        data["TSE"].pop("bid", None)
        data["TSE"].pop("ask", None)
...
# === 7. 期貨 tag 判斷 helper ===============================
def fut_tag(code: str) -> str | None:
    if code.startswith("TXF"):
        return "TXF"
    if code.startswith(("MXF", "MX4")):
        return "MXF"
    return None
...
def ensure_sub():
    ...
    target = set(ks[i:i+15] + ks[max(0, i-25):i])
    added = False
    for k in target:
        for cp in ("C", "P"):
            c = idx.get(k, {}).get(cp)
...
def on_tick(_: Exchange, t: TickFOPv1):
    ...
    # 選擇權
    if t.code not in code2exp:
        return
    sio.emit("optionData", {
        "code": t.code,
        "strike": strike(t.code),
        "expiration": code2exp[t.code],
        "cp": cp_of(t.code),
        "last": float(t.close),
        "change_rate": float(getattr(t, "change_rate", 0)),
...
def on_ba(_: Exchange, ba: BidAskFOPv1):
    tag = fut_tag(ba.code)
    if tag:
        bid = float(ba.bid_price[0] or 0)
        ask = float(ba.ask_price[0] or 0)

        # 共用 bid / ask
        if bid:
            mkt_state[tag]["bid"] = bid
        if ask:
            mkt_state[tag]["ask"] = ask

        # ---- TXF：不修改 last，保留 tick 成交價 ----
        if tag == "TXF":
            emit_mkt()
            return

        # ---- MXF：用 bid+ask 中間值，並取整數 ----
        if tag == "MXF":
            if bid and ask:
                mid = int(round((bid + ask) / 2))      
                mkt_state[tag]["last"] = mid
            elif bid or ask:                              # 只有一邊時取該價
                mkt_state[tag]["last"] = int(bid or ask)
            emit_mkt()
            return

    # -------- 選擇權 Bid/Ask --------
    if ba.code not in code2exp:
        return
    getp = lambda arr, i: float(arr[i]) if arr and len(arr) > i and arr[i] else None
    sio.emit("bidAskData", {
        "code": ba.code,
        "strike": strike(ba.code),
        "expiration": code2exp[ba.code],
        "cp": cp_of(ba.code),
