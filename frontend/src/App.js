// -----------------------------------------------------------
// App.js  (fixed – panel scroll & persistent hover)
// -----------------------------------------------------------
...
const tradableMinutesUntil = exp => {
  ...
  while(cur<end){
    ...
    if(wd===6&&cur>=new Date(y,m,d,5)){cur=new Date(y,m,d+2,8,45);continue;}
...
export default function App(){
  ...
  const expirations = useMemo(
    ()=>[...new Set(rows.map(r=>r.expiration))].sort(),[rows]
  );
  ...
  const currentPoint = useMemo(
    ()=>[tradableMinutesUntil(nearExp),otmVal],[nearExp,otmVal]
  );

  /* ---------- InfoCard ---------- */
  const InfoCard = ({title,data})=>(
    <div className="info-card" title={`${title}：即時成交；滑鼠停留可看買賣價`}>
